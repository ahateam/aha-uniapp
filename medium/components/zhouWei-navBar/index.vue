<template>
	<view>
		<view class="header" :class="{fixed:navFixed,absolute:type == 'transparent','line':navLine,colorWhite:isWhite}"
		 :style='{height: (44 + statusBarHeight)+"px",background: navBgColor,color:navFontColor,opacity:transparentValue}'>
			<view class="left_box">
		...
		<view class="header transparentFixed fixed" :class="{colorWhite:isWhite}" v-if="type == 'transparentFixed'" :style='{height: (44 + statusBarHeight)+"px",color:navTransparentFixedFontColor,opacity:1 - transparentValue,zIndex:transparentValue < 0.3 ? 100 :90 }'>
			<view class="left_box">
...
<script>
	...
	const mainPagePath = ["pages/task/task"];
	//返回首页的地址
	const homePath = "/pages/task/task";
	//白色表达值
