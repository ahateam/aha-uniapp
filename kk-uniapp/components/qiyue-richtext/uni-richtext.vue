		...
		<view class="content">
			...
			<view ref="richtext" v-for="(item,index) in richList" :key="index" :style="index==richListIndex?'background:#cce0f2;':''"
			 @click="showInputPopup(index,item)">
				<view v-if="item.type=='textarea'" v-html="item.value" :style="item.style"></view>
				<image v-if="item.type=='img'" mode="widthFix" :src="item.value"></image>
				<video v-if="item.type=='video'" :src="item.value" controls></video>
			</view>
		</view>
	</view>
</template>
