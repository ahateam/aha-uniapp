		...
		<swiper :current="tabCurrentIndex" class="swiper-box" duration="300">
			<swiper-item class="tab-content" v-for="(tabItem,tabIndex) in navList" :key="tabIndex">
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<!-- 订单列表 -->
					<view v-for="(item,index) in contentList" :key="index" class="order-item">
						...
						<view class="goods-box-single">
							...
							<view class="right">
								...
								<text class="attr-box">{{item.attr}} x 1</text>
	...
	export default {
		...
		onLoad(options) {
			...
			let cnt = {
				...
				// storeId: storeId, // Long <选填> 商店编号
				buyerId: uni.getStorageSync('userId'), // Long <选填> 买家用户编号
				// pramoterId: pramoterId, // Long <选填> 卖家推广员用户编号
				// resellerId: resellerId, // Long <选填> 分销用户编号（只记录直接上级
				// title: title, // String <选填> 标题
				// channelId: channelId, // String <选填> 渠道编号
				count: 100, // int 
				...
				this.$api.getOrders(cnt,(res)=>{
					if(res.data.rc == this.$util.RC.SUCCESS){
						...
						for(let i = 0;i<data.length;i++){
							...
							let d = time.getDate()
							let hours = time.getHours(); //获取系统时，
							let minutes = time.getMinutes(); //分
							// let s = time.getSeconds(); //秒
							let newTime = `${y}-${m}-${d} ${hours}:${minutes}`
							data[i].time = newTime
							
							let img = this.$util.tryParseJson(data[i].product.data)[0].img
							data[i].image = img
							
							let attr = this.$util.tryParseJson(data[i].sku)
							let sku = `${attr.color} ${attr.size}`
							data[i].attr = sku
							data[i].price = attr.price
						}
