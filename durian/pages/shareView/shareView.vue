<template>
	<view>
		<view class="bg-view">
			<view class="share-top" @click="navToPost" v-if="posterImg">
			...
			<view class="share-top flex-box" v-else><loading-icon title="海报生成中"></loading-icon></view>
			...
			<view class="bottom-box">
				...
				<view class="bottom-list">
					<view class="bottom-list-box" v-for="(item, index) in shareList" :key="index" @click="shareBtn(index)">
						<view class="bottom-img"><image :src="item.imgSrc" mode="aspectFit"></image></view>
						<view>{{ item.name }}</view>
					</view>
				</view>
				<button class="close-btn" @click="navBack">取消</button>
			</view>
		</view>
...
import LoadingIcon from '@/components/Loading/LoadingIcons.vue';
...
export default {
	...
	methods: {
		...
		shareFind() {
			uni.navigateTo({
				url: `/pages/find/createView/createView?shareType=${this.shareType}`
			});
		},
			...
			this.$api.getUserInfo(cnt, res => {
				if (res.data.rc == this.$util.RC.SUCCESS) {
					...
					let shareInfo = {
						...
						shareText: userInfo.userName,
						shareImg: userInfo.userHead,
						shareType: this.$constData.shareType[0].key
					};
					this.$store.commit('updateShareInfo', shareInfo);
				} else {
					uni.showToast({
						title: res.data.rm,
						icon: 'none'
	...
	onLoad(res) {
		...
		if (res.shareType == this.$constData.shareType[0].key) {
			let cnt = {
				userId: uni.getStorageSync('toUserId') // Long 用户id
			};
			this.getUserInfo(cnt);
		} else if (res.shareType == this.$constData.shareType[1].key || res.shareType == this.$constData.shareType[2].key) {
			let shareId = this.$store.state.share.shareInfo.shareId;
			this.shareText = this.$store.state.share.shareInfo.shareText;
			if (res.type == this.$constData.groupType[3].key) {
				this.val = `/pages/find/videoView/videoView?id=${shareId}`;
			} else {
				this.val = `/pages/find/imgView/imgView?id=${shareId}`;
			}
		}
	}
};
...
.bg-view {
	background-color: rgba($color: #010101, $alpha: 0.5);
	width: 100%;
	height: 100vh;
}
...
.bottom-img {
	...
	height: 100rpx;
	margin-bottom: 14rpx;
	background-color: rgba($color: #f2f5f7, $alpha: 0.5);
	border: 1rpx solid rgba($color: #cfdce9, $alpha: 0.5);
