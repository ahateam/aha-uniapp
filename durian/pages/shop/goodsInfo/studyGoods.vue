<template>
	<view>
		...
		<view class="bannerBox">
			...
			<view class="dotBox"><view class="swiperDot" v-for="(item, index) in imgList" :key="index" :class="current == index ? 'currDot' : ''"></view></view>
		</view>

		<view class="title-box">{{ title }}</view>

		<view class="price-box">
			<text style="margin-right: 0.3em;">AUD</text>
			{{ price }}
		</view>
		...
		<view class="infoBox">{{ text }}</view>

		<view class="bottom-box">
			<button class="left-btn">
				<image class="mes-icon" src="/static/image/icon/icon-mes.png" mode="aspectFit"></image>
				联系卖家
			</button>
			<button class="right-btn" @click="buyStatus = !buyStatus">
				<image class="mes-icon" src="/static/image/icon/icon_shop.png" mode="aspectFit"></image>
				立即购买
			</button>
		</view>
...
.currDot {
	width: 22rpx;
	background-color: rgba($color: $group-color-w, $alpha: 1);
}
