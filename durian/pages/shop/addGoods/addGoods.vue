<template>
	<view>
		<navBar :back="false" type="transparent" style="position: fixed;width: 100vw;background-color: #FFFFFF;" fontColor="#000">
			<image slot="left" class="back-icon" src="/static/image/icon/icon_fh.png" mode="aspectFit" @click="navBack"></image>
			<view class="title-box">发布商品</view>
			<view slot="right" class="add-tbn" @tap="addGoods">发布</view>
		</navBar>
		<view class="padding-nav"></view>
		<scroll-view scroll-y scroll-with-animation class="scroll-view">
			<view class="autoTitle">商品名称</view>
			...
			<view class="imgBox">
				<view class="imgs" v-for="(item, index) in imgList" :key="index" v-if="index < 3">
					<image :src="constData.oss + item" mode="aspectFill" :style="index == 2 ? 'margin-right:0' : ''"></image>
					<view class="iconfont icon-jia delBtn" @click="delImg(index)"></view>
				</view>
				<view class="addImgBtn" @click="addImgs" v-if="imgList.length < 3"><text class="iconfont icon-jia"></text></view>
			</view>

			<view class="lineBox"></view>

			<view class="autoTitle" style="margin-top: 30rpx;">价格</view>
			<input class="autoInput" type="number" v-model="money" placeholder="请输入商品价格" />

			<view class="autoTitle">商品描述</view>
			<view class="autoInput areaBox">
				<textarea placeholder="请输入商品简介" v-model="info" maxlength="200" />
				<view class="infoNumber">
					(
		...
		createGoods(cnt) {
			this.$api.createGoods(cnt, res => {
				if (res.data.rc == this.$util.RC.SUCCESS) {
					...
					console.log(data);
					let shareInfo = {
						shareId: data.goodsId,
						shareType: this.$constData.shareType[7].key,
						shareImg: this.$util.tryParseJson(data.goodsData)[0],
						shareText: data.goodsName,
						ext: data.goodsPrice
					};
