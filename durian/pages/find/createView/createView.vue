<template>
	<view>
		<navBar :back="false" v-if="!imgUrl">
			...
			<view slot="right" class="rightBtn"><button type="primary" @click="addContent">发表</button></view>
		</navBar>
		...
		<movable-area class="imgBox" v-if="!imgUrl">
			<view class="textBox"><textarea v-model="text" placeholder="告诉大家你今天的分享…" /></view>

			<view class="video-box" v-if="videoSrc"><video :src="constData.oss + videoSrc" controls></video></view>

			<view style="margin: 0 30rpx;" v-if="shareInfo.shareText"><share-box :shareInfo="shareInfo"></share-box></view>
		...
		<uni-popup class="popup-box" :show="showPopup" type="bottom" mask-click @change="change">
			<button class="chose-btn" v-for="(item, index) in choseImg" :key="index" @click="addImgs(item.type)">{{ item.name }}</button>
...
export default {
	...
	methods: {
		...
		addContent() {
			if (!this.uplogin) {
				...
				} else if (this.shareInfo.shareText) {
					cnt.type = this.$constData.groupType[4].key;
					cnt.data = JSON.stringify(this.shareInfo);
				} else if (this.imgList.length == 0) {
	...
	onLoad(res) {
		if (res.shareType == 0 || res.shareType) {
			this.shareInfo = this.$store.state.share.shareInfo;
		}
...
.del-img-tip {
	...
	width: 100%;
	height: 140rpx;
	background-color: rgba($color: #ee455a, $alpha: 0.9);
	color: $group-color-w;
	font-size: $group-font-befor;
