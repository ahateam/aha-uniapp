		...
		<view v-if="pageStatus != 'onload'">
			...
			<view class="navBox auto-margin">
				<view class="navList" v-for="(item, index) in navList" :key="index" :style="index == 1 ? 'margin-right:0;' : ''" @click="changeNav(index)">
					...
					<!-- 選中样式 -->
					<view class="currNav" :class="{ displayBox: index == navCurrtent }"><image src="/static/image/find/icon_xt.png" mode="aspectFit"></image></view>
					<!-- the end -->
				</view>
			</view>
			...
			<view class="tagBox auto-margin">
				<view
					class="tagsList"
					:class="{ currTag: index == navList[navCurrtent].tagCurrtent }"
					v-for="(item, index) in navList[navCurrtent].tagList"
					:key="index"
			...
			<view>
				<view class="listBox" :class="{ autoList: index != 0 }" v-for="(item, index) in contentList" :key="index" @click="navToContent(item)">
					...
					<view v-else>
						<right-video
							v-if="item.posting.postingType == constData.groupType[1].key || item.posting.postingType == constData.groupType[3].key"
							:item="item"
							:src="constData.oss + getJsonParse(item)"
							:listLength="getJsonParse(item, true)"
							:tagType="navList[navCurrtent].tagCurrtent == 0 ? 'new' : 'j'"
						></right-video>

						<only-text
							:item="item"
							v-else-if="item.posting.postingType == constData.groupType[0].key"
							:tagType="navList[navCurrtent].tagCurrtent == 0 ? 'new' : 'j'"
						></only-text>
...
export default {
	...
	methods: {
		...
		shareBtn(item, index) {
			...
			let shareInfo = {
				shareId: item.posting.postingId,
				shareText: item.posting.postingTextDate,
				shareImg: this.$util.tryParseJson(item.posting.postingDate)[0],
				type: item.posting.postingType,
				shareType: shareType
			};
			console.log(this.$store.state.share.shareInfo);
			this.$store.commit('updateShareInfo', shareInfo);

			uni.navigateTo({
				url: `/pages/shareView/shareView?shareType=${shareType}`
			});
		},
