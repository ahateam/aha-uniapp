<template>
	<view>
		<view
			class="contentList"
			:class="{ 'show-time': showTime && showTime && item.goodsType == constData.goodsType[0].key }"
			:style="item.goodsType == constData.goodsType[0].key ? cardBg : ''"
			v-for="(item, index) in list"
			:key="index"
			@touchstart="change(index, item)"
			@touchend="changeEnd(index, item, true)"
			@click="navToInfo(item)"
		>
			<!-- 平台商品背景图片    -->
			<image
				v-if="item.goodsType != constData.goodsType[0].key"
				class="card-img"
				:src="item.goodsType == constData.goodsType[1].key ? cardImgSrcY : cardImgSrcB"
				mode="aspectFill"
			></image>
			<image
				v-if="item.goodsType != constData.goodsType[0].key"
				class="card-img-tag"
				:src="item.goodsType == constData.goodsType[1].key ? cardImgSrcYbg : cardImgSrcBbg"
				mode=""
			></image>
			<!-- 标题 -->
			...
			<!-- 自由商品图片 -->
			<view class="stuty-img" v-if="item.goodsType == constData.goodsType[0].key"><image :src="constData.oss + getImgSrc(item.goodsData)" mode="aspectFill"></image></view>
...
.contentList {
	...
	&:active {
		background-color: rgba($color: #fffcf3, $alpha: 1);
	}
}
