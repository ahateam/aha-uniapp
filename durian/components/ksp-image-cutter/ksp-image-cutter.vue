<template>
	<view v-show="url" class="ksp-image-cutter">
		...
		<view class="body">
			<image v-if="url" class="image" @load="imageLoad" :style="{left: image.left + 'px', top: image.top + 'px', width: image.width + 'px', height: image.height + 'px'}"
			 :src="url"></image>
			<view v-if="mask.show" class="mask"></view>
			<view @touchstart="touchStart($event, 'plank')" @touchmove="touchMove" @touchend="touchEnd" @touchcancel="touchCancel"
			 class="plank">
				<view class="frame" @touchstart="touchStart($event, 'frame')" @touchstart.stop.prevent="touchHandle" :style="{left: frame.left + 'px', top: frame.top + 'px', width: frame.width + 'px', height: frame.height + 'px'}">
					<canvas v-if="mask.show" class="canvas" :style="{width: frame.width + 'px', height: frame.height + 'px'}"
		...
		<view class="toolbar">
			<button @tap="oncancle" class="btn-cancel">取消</button>
			<button @tap="restart" class="btn-cancel">还原</button>
			<button @tap="onok" class="btn-ok">完成</button>
		</view>
	</view>
</template>
		...
		methods: {
			...
			trimImage() {
				...
				query.select(".body").boundingClientRect((data) => {
					...
					var tx = (bw - tw) / 2;
					var ty = (bh - th) / 2;
					var ax = tx - this.frame.left;
					var ay = ty - this.frame.top;
					this.frame.width = tw;
					this.frame.height = th;
					this.frame.left = tx;
					this.frame.top = ty;
...
<style lang="scss" scoped>
	...
	.frame-left-top {
		...
		left: 0;
		top: 0;
		border-left: 4rpx solid #00C8BE;
		border-top: 4rpx solid #00C8BE;
	}
	...
	.frame-left-bottom {
		...
		bottom: 0;
		border-left: 4rpx solid #00C8BE;
		border-bottom: 4rpx solid #00C8BE;
	}
	...
	.frame-right-top {
		...
		right: 0;
		top: 0;
		border-right: 4rpx solid #00C8BE;
		border-top: 4rpx solid #00C8BE;
	}
